...
### 🔧 Infra / Ops
- **compose/vllm (quality 모델 교체 가능)**: 최종 LLM을 단일 env **`QUALITY_MODEL`**(HF repo id 또는 로컬 경로)로 지정 — HF id면 `hf-cache` 볼륨으로 자동 다운로드, 로컬 경로면 `./models` 마운트에서 로드. 기본 **`Qwen/Qwen3.5-4B`**. `--model`·`--served-model-name`·게이트웨이 라우팅 키를 한 값으로 통일. **gated 모델**은 `.env`의 `HF_API_KEY`→`HF_TOKEN` 매핑으로 다운로드. draft(HY-MT)는 고정.
