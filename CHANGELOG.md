...
### 🔧 Infra / Ops
- **compose/vllm (quality 모델 교체 가능)**: 최종 LLM 로드 소스를 `.env`로 분리 — `QUALITY_MODEL_ID`에 **HF repo id**를 주면 `hf-cache` 볼륨으로 자동 다운로드, 로컬 경로면 `./models` 마운트에서 로드(기본 Qwen3-4B). 서빙 별칭(`QUALITY_MODEL`=게이트웨이 라우팅 키)·컨텍스트(`QUALITY_MAX_MODEL_LEN`)·GPU 점유(`QUALITY_GPU_UTIL`)도 env화. **gated 모델**(예: Gemma)은 `.env`의 `HF_API_KEY`→컨테이너 `HF_TOKEN` 매핑으로 다운로드. draft(HY-MT)는 고정이라 로컬 마운트 유지.
- **compose/vllm (HY-MT 초벌 튜닝)**: draft 서버를 HY-MT1.5-1.8B 공식 배포 가이드(vLLM≥0.10)에 맞춰 튜닝 — `--dtype bfloat16`(공식 권장) 명시, `--max-num-seqs 32`(초벌 single-flight 저지연·KV 여유), prefix-caching 유지(타이핑 prefix + Pombal 컨텍스트 재사용). vLLM 이미지를 `latest`→**`v0.25.1` 핀**(재현성). **HY-MT 공식 `repetition_penalty=1.05`**를 draft 엔진에 기본 주입(`extra_body`, `config.draft_repetition_penalty`) — **실측**: 지연 오버헤드 ≈0·충실한 반복 번역 무손상(1.5/2.0은 문법 왜곡 → 1.05가 안전 기본값), 컨텍스트 prefix 재사용 시 TTFT ~23→~11ms(prefix-caching). temp=0에서 degenerate 루프는 미발생 → 공식 config 정합 + tail-risk 보험.
