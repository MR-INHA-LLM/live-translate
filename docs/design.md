...
데모 기준은 **엄밀한 증명이 아니라 설득력 있는 시각화**다(단, 값은 실제 계산 —
fake 금지). 설득력 순:

| 장치 | 무엇을 보여주나 | 우선순위 |
|---|---|---|
| **구 정렬 하이라이팅** | 소스 구 hover/자동 → 대응 번역 구가 같은 색으로 이어짐. "조각조각 정확히 옮겼다"가 한눈에 | **센터피스** (가장 설득력) |
| **단어 QE 색상** | 단어들이 초록빛(불확실만 amber). 시각적 안심 | **주력** (CometKiwi 파생, M4) |
| **witness 언어(en)** | 못 읽는 타겟 대신 읽을 수 있는 언어의 독립 forward 번역 | **주력** |
| **역번역(id→ko)** | "다시 돌리니 원문 그대로" — lay user에게 가장 직관적 | **주력** |
| 숫자 COMET/QE | 전문가·디버그 패널에만 | lay엔 부적합 |

- **구 정렬**은 사전학습 정렬 모델 **awesome-align**(`aneuraz/awesome-align-with-co`,
  학습 불필요)으로 (소스, 최종) 쌍에서 **턴 확정 시 1회** 계산 → 타이핑 지연과 무관,
  캐시. 실측상 내용어 정확·`그거→that`까지 잡음(decisions.md D13). ONNX 버전이면
  torch/transformers 없이 실행(충돌 없음). 전부 실제 계산이라 뜯어봐도 성립.
- 정직성 가드(§8.5)는 유지하되 데모 톤은 설득력 우선(근거 D13).
