...
```
┌─────────────────────────────────────────────────────────────┐
│  언어  [ 한국어 ⇄ 영어 ]   ← 기본 ko⇄en · ⇄로 방향 스왑       │  ← 양방향 선택기
├──────────────┬────────────────────────┬────────────────────┤
...
> **언어 선택기는 양방향** `[A] ⇄ [B]`. 기본 `ko ⇄ en`. `⇄`로 방향을 뒤집는다
> (ko→en ↔ en→ko). 아래 시나리오는 witness가 빛나는 `ko ⇄ id`로 바꾼 경우다 —
> 확인용(en) 열은 상대 언어를 못 읽는 조합(예: id)에서 켜진다. (위 예시는 ko⇄id 전환 상태)
...
flowchart LR
  ...
  D --> C
  C -->|"Enter로 확정"| E["최종 번역 표시<br/>(선명 · 맥락 반영, 초벌과 나란히)"]
  E --> B
```
