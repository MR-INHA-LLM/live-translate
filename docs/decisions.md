...
- temp=0은 완전 결정적(동일 프롬프트 5/5 동일) → hold-k의 재현성 전제 자체는 성립.
- 그러나 **ko(SOV)→id(SVO)는 소스가 한 어절 늘 때마다 목표문이 전면 재작성**되어
  이전 번역 대비 접두어 생존율이 대부분 0%. "연속 k회 동일 접두어 확정"으로는
  확정할 접두어가 생기지 않는다.

→ **접두어 freeze는 flicker 해법이 아니다.** 완화는 (a) 디바운스(어절 경계),
(b) 미확정 draft 전체를 tentative로 렌더, (c) revision_id 순서제어/이전요청 abort로
간다. 목표문 접두어 확정은 어순이 유사한 쌍에 한해 선택적으로만. (README §2.1)

### D4. 레이턴시 — 추론은 병목이 아니다

초벌 TTFT p50 12.5ms / Total p50 91ms로 목표(150/400ms)를 ~10배 상회.
목표 초과는 콜드 첫 요청(TTFT 222ms)뿐 → 세션 워밍업 1회로 제거.
→ **레이턴시 예산은 디바운스(150~250ms)가 지배한다.** 튜닝 레버는 모델 속도가
아니라 안정화 정책. (README §2.1, §4)
